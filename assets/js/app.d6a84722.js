(() => {
  ...
  function downloadCategoryFiles(subsetId) {
    ...
    // One click used to fire one download per file. Browsers block bulk
    // programmatic downloads after the first, and on the anonymous host every
    // file is a separate rate-limited request, so most of them failed. Hand
    // over a URL list instead: one download, then `wget -i <file>`.
    const urls = files.map(item => new URL(toAssetUrl(item.path), window.location.href).href);
    const blob = new Blob([`${urls.join("\n")}\n`], { type: "text/plain" });
    const blobUrl = URL.createObjectURL(blob);
    triggerBlobDownload(blobUrl, `amam-${subsetId}-urls.txt`);
  ...
  function bindEvents() {
    ...
    els.downloadAllBtn.addEventListener("click", () => {
      const link = window.AmamAssetUrl.toRepoZipUrl();
      if (link) {
