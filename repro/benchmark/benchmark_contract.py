"""Shared release-bundle identities used by benchmark validators."""

from __future__ import annotations


EXPECTED_PAIRS = 128

EXPECTED_DEEP_MODEL_ORDER = (
    "dl_unet_r34",
    "dl_unetpp_r34",
    "dl_deeplabv3_r34",
    "dl_deeplabv3p_r34",
    "dl_fpn_r34",
    "dl_pspnet_r34",
    "dl_linknet_r34",
    "dl_manet_r34",
    "dl_segformer_b0",
    "dl_upernet_b0",
    "dl_segformer_b2",
    "dl_upernet_b2",
    "dl_unet_effb0",
    "dl_deeplabv3p_effb0",
    "metal_unet_gray_r34",
    "metal_unet_clahe_r34",
    "metal_unet_edge4_r34",
    "metal_unet_gabor_r34",
    "metal_unet_lbp_r34",
    "metal_unetpp_gray_r34",
    "metal_deeplabv3p_clahe_r34",
    "metal_fpn_gabor_r34",
    "metal_linknet_edge4_r34",
    "metal_segformer_clahe_b0",
    "metal_upernet_clahe_b2",
    "metal_segformer_gray_b2",
    "metal_manet_edge4_effb0",
    "metal_unetpp_clahe_effb0",
    "metal_mlography_unet_vgg16_gray",
)
EXPECTED_DEEP_MODEL_IDS = frozenset(EXPECTED_DEEP_MODEL_ORDER)

CANONICAL_DEEP_PREDICTION_MODELS = frozenset(
    {"dl_unet_effb0", "metal_unetpp_clahe_effb0"}
)
