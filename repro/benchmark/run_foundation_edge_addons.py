...
def get_device(requested: str) -> torch.device:
    ...
    try:
        ...
        raise ValueError(
            f"Invalid --device value {requested!r}; use auto, cpu, cuda, or cuda:<index>."
        ) from exc
    ...
    if device.type == "mps" and not (
        ...
        raise RuntimeError("MPS was requested, but it is unavailable in this PyTorch environment.")
    return device


def validate_texturesam_assets(repo_dir: Path, checkpoint_path: Path) -> None:
    sam2_root = repo_dir / "sam2"
    if not sam2_root.exists():
        raise FileNotFoundError(
            "TextureSAM is required for the complete 45-model reproduction, but its repo was not found: "
            f"{sam2_root}\n"
            "Expected source: https://github.com/Scientific-Computing-Lab/TextureSAM"
        )
    if not checkpoint_path.exists():
        raise FileNotFoundError(
            "TextureSAM is required for the complete 45-model reproduction, but its checkpoint is missing: "
            f"{checkpoint_path}\n"
            "Expected source: https://drive.google.com/drive/folders/1pUJLa898WYEcb4Y_sOaXsSVe-CsPkwRv"
        )


def load_texturesam_generator(
    repo_dir: Path,
    checkpoint_path: Path,
    device: torch.device,
):
    validate_texturesam_assets(repo_dir=repo_dir, checkpoint_path=checkpoint_path)
    sam2_root = repo_dir / "sam2"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--preflight-only",
        action="store_true",
        help="Validate the device and required external assets without loading models.",
    )
...
def main() -> None:
    ...
    texturesam_repo = Path(args.texturesam_repo)
    texturesam_ckpt = Path(args.texturesam_ckpt)
    if "texturesam_03" in wanted:
        validate_texturesam_assets(repo_dir=texturesam_repo, checkpoint_path=texturesam_ckpt)

    print(f"[info] device={device}")
    if args.preflight_only:
        print(f"[preflight] selected models: {','.join(m for m in all_model_ids if m in wanted)}")
        print("[preflight] foundation/edge device and external assets are ready")
        return
